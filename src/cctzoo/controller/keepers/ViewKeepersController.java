    ...
    protected void updateButtonController() {
        viewKeepers.getUpdateKeeperButton().addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                updateKeeper = new UpdateKeeperFrame("Admin - Update Zoo Keeper", 975, 640, "src/cctzoo/view/images/animal.png");
                ...
                ArrayList<Animal> lessAnimals = new ArrayList<>();
                lessAnimals.clear();
                lessAnimals = model.getListAnimals();

                ArrayList<Animal> animalsAllowed = searchEngine.searchAnimalByKeeper(lessAnimals, model.getListKeepers().get(index).getQualification());

                int w = 0;
                updateKeeper.getAnimalsPanel().dataToString(animalsAllowed);
                updateKeeper.getAnimalsPanel().setData();
                for (int j = 0; j < model.getListKeepers().size(); j++) {
                    for (Animal ak : model.getListKeepers().get(j).getkeeperAnimals()) {
                        for (int i = 0; i < updateKeeper.getAnimalsPanel().getViewAnimalsTable().getRowCount(); i++) {
                            if (ak.getExhibitionNumb() == Integer.parseInt((String) updateKeeper.getAnimalsPanel().getViewAnimalsTable().getValueAt(i, 0))) {
                                w++;
                                updateKeeper.getAnimalsPanel().removeRow(i);
                            }
                        }
                    }
                }
                ...
                ctrl.getUpdateKeeperCtrl().setIndex(index);
